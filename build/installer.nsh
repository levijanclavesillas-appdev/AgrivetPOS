; NSIS customisation for ChachiPOS — TASK-018 requirement 6.
;
...
; already stops it deleting %LOCALAPPDATA%\ChachiPOS, and the backup folder is
...
!macro customUnInstallCheck
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Remove Chachi POS from this computer?$\r$\n$\r$\n\
     Your data is NOT removed:$\r$\n$\r$\n\
     • The database stays in$\r$\n\
       $LOCALAPPDATA\ChachiPOS$\r$\n$\r$\n\
...
!macro customRemoveFiles
  ...
  Delete "$INSTDIR\*.*"
  RMDir "$INSTDIR"
  ; $LOCALAPPDATA\ChachiPOS is deliberately absent from this macro.
