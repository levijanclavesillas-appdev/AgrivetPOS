...
// the answer is *per row* — a rejected row is named with the tab and the row number the
// operator scrolls to in their own spreadsheet, because "3 rows are invalid" against a
// 500-row catalogue is not something anybody can act on. It lives in
// `shell/opening.js` since TASK-047, because the setup wizard offers the same load to a
// store that has just been installed.
...
export function createData({ root, session }) {
  ...
  // The opening load keeps its own state, apart from the archive import's: the two
  // halves of the screen are separate conversations and neither should reset the other.
  // Its element is made once and re-attached on every render, so re-drawing the import
