...
| [TASK-045](TASK-045-material-design-system.md) | A design system, a screen that fits, and three transitions to anything | *this commit* | `public/css/tokens.css` — Material 3 as tokens, on the platform palette `04_UX_SPEC.md` had cited since v2.0 and the stylesheets had never adopted. The shell became the viewport and `.screen` the only scroll container, so the document stopped scrolling on five screens — `SCR-702` was 6,941 px in one column. `SCR-601` gained an index of every report and `SCR-201` a low-stock filter; `TC-UI-13` walks the navigation per role and holds it to three transitions. It found that the low-stock list was unreachable by the inventory clerk, whose job it is |
...
**And one that came from the other direction.** `TASK-045` was not in any backlog: the product had
eight stylesheets, four radii and a green nobody had specified, while `04_UX_SPEC.md` described a
blue one. Closing that meant measuring the screens, and measuring them found five that did not fit
the 1366×768 the same document calls the design target — one of them nine viewports tall. Counting
the navigation while there found a screen the role it exists for could not open at all.
...
| [TASK-048](TASK-048-google-sign-in-and-subscription.md) | Google sign-in and a subscription checked online once a month | **Live**, *this commit*: the licence server at `pos.chachisoftware.store` (Docker), and the POS builds enforce it | Each store's POS linked by its owner before it updates; a Play Console subscription and service account; Play Billing in the Android app |
...
| [TASK-057](TASK-057-new-computer-restore.md) | A store moves to a new computer with its backup: in the wizard, or from the Backups tab | Built, `0c7317a` | The store's next change of computer |
| [TASK-058](TASK-058-account-and-customer-screens.md) | Sign out, your own password and PIN, the recovery code, and editing a customer | Built, `aadbea9` | Staff changing the passwords the owner set for them |
...
| [TASK-072](TASK-072-restocking-requests.md) | **Restock** (`SCR-806`): what to buy, netted off what is already on order, asked for and approved, converted to one draft order per supplier (`PO-107`–`PO-109`). Found that a product with no minimum was invisible to every low-stock surface, at zero stock | Built, *this commit* | The store's first week of buying from it |
| [TASK-073](TASK-073-startup-and-idle-performance.md) | **Slow before it has anything to be slow about**: `bcryptjs` at 1358 ms a hash against a documented 300 ms, 57 modules to draw a login box, no cache headers, a counter search without a debounce | Items 1–5 built, *this commit* (module load 1474 ms → 798 ms); 6–10 open | Start-up timed on the store's own machine (`NFR_4.1`) |
