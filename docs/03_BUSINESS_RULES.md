...
| `AUD-603` | Every authorisation override — discount above ceiling, over-limit credit, below-cost sale, expired-stock sale, over-receipt, cost variance, restock against default, late return, sale void, stale stock count — records the **requesting user and the approving user as distinct actors**, with the reason. | 1.0 |
