...
| `POST` | `/batches/:id/recall` | `TX-407` | `INV-208`: hold the batch, with a `reason`. FEFO skips it and a sale needing it is refused |
| `POST` | `/batches/:id/recall/lift` | `TX-407` | `INV-208`: release it, with a `reason` |
| `POST` | `/batches/:id/return-to-supplier` | `TX-407` | `INV-208`: all that a recalled batch holds, by one `SUPPLIER_RETURN` movement; optional `note` |
...
| `SEC-8` | **Local exposure** | Express binds `127.0.0.1` in v1.0. It is never bound to `0.0.0.0` until v1.3 adds device authentication — a POS API on an open LAN with no client auth is a store-wide compromise. **TASK-062, the web version:** a hosted copy (`AGRIVET_HOSTED=1`, `src/config/hosting.js`) listens on `0.0.0.0` *inside its container*, which compose publishes on the host's `127.0.0.1` only; nginx with TLS is the way in. Its setup wizard needs a one-time code (`AGRIVET_SETUP_CODE`), sign-in is rate-limited per address by nginx on top of `SEC-3`, and every response carries `X-Frame-Options: DENY`, `frame-ancestors 'none'`, `nosniff` and `no-referrer`. On a store's PC nothing changes. |
