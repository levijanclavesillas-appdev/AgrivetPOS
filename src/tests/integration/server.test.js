...
test('GET /api/v1/health/panel reports OPS-006’s six figures, behind TX-428', async () => {
  ...
  assert.deepEqual(Object.keys(body.database.row_counts).sort(), [
    ...
    'customer_credit_accounts', 'customer_credit_transactions', 'customer_prices', 'customers',
    'goods_receipt_items', 'goods_receipts',
    'inventory', 'inventory_movements', 'licence_state', 'open_orders', 'payment_reconciliations',
    'product_barcodes', 'product_batches', 'product_images', 'product_pack_prices', 'product_packs', 'product_prices',
    'product_quantity_breaks', 'products',
    'purchase_order_items', 'purchase_orders', 'quick_keys',
    'restock_request_items', 'restock_requests',
...
test('the renderer is served from the same origin as the API', async () => {
  ...
  assert.equal(res.status, 200);
  assert.match(await res.text(), /Chachi POS/);
});
