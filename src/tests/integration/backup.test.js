...
test('the archive is a real zip, readable by something that did not write it', () => {
  ...
  assert.match(result.file_name, /^chachipos_backup_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_manual_[0-9a-f]{6}\.zip$/);
...
test('TC-INT-74: a restore needs the filename typed, exactly', () => {
  ...
  for (const typed of ['', null, 'chachipos_backup.zip', backup.file_name.slice(0, -1), backup.file_name.toUpperCase()]) {
    assert.throws(
